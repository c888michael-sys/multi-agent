...
3. **Response** — composer **A** pins to the bottom of the stage. The **newest** response **B** sits at the **vertical and horizontal center** of the stage area between A and the hint; on every new submit, the new B takes the center slot with a pop-out animation, and the previously-newest B becomes one of the **older B's** stacked just above it, fading upward via a top-edge mask. The top of the stack is a **hint bar** ("pull or tap // expand the mindmap"). Each B card is flat, terminal-style: a small `›` prompt echo, a synthesized-text body, a copy chip on the foot — no left rail, no chat-bubble feel.
...
5. **Mindmap (orbital)** — the singularity expands into an orbital cluster: the **active composer A** lands at the geometric center of the stage; each categorized chunk of the newest response (sections / files / targets / phases) becomes an orbital **node** positioned around A via polar coords with a small random angle + radius jitter for a loose force-directed feel. An SVG layer draws a dashed accent line from each node back to A. Nodes drift gently in place via per-node CSS animation (small translate, 6–10 s alternate). A scanline overlay + the `// A · COMPOSER` tag drive the "agent" read. **Submitting a new prompt from A while in this phase** transitions back to *loading* and adds the new B to the stack — the orbital is just another way to compose follow-ups, not a dead-end view. Click *collapse* to implode back to the response stack.
6. **Imploding** (transient ~440 ms) — collapse-back animation; nodes converge to center, lines fade, the composer scales down, then the response stack re-materializes.
...
7. Click (or pull-drag-down past ~70 px) the hint bar. **Expected:** the whole stack — every B and the composer — converges toward a singularity at stage-center over ~520 ms; then it expands into an **orbital cluster**: the composer A sits at the geometric center, the categorized chunks of the newest response become nodes positioned around A, and dashed accent lines connect each node back to A. Nodes drift gently in place.
...
**Layout note — response stack + orbital mindmap (current).** The web UI uses a stacked response model with the **newest B at vertical center** of the stage area, older B's stacked above it with a top-edge fade mask, and composer A pinned at the bottom; the hint sits at the top of the stack. Pulling the hint runs a collapse-into-singularity animation (520 ms) followed by an **orbital mindmap**: composer A at the geometric center of the stage, the newest response's categorized chunks (sections / files / targets / phases) as nodes positioned around A via polar coords with random jitter, and SVG dashed lines from each node back to A. Submitting from the center composer in mindmap mode adds a new B to the stack. The CSS keyframes driving this live in the *Response stack* / *Big bang collapse* / *Orbital mindmap* blocks of `src/web/static/style.css`; the orbital geometry / sizing logic is in the `useOrbitalPositions` hook in `src/web/static/app.jsx`. The response history is the full ordered stack, persisted under `localStorage[lattice.responseStack.v2]`. The earlier `lattice.lastThread.v1` key is dead.
