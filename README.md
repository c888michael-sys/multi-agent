...
- [x] **Round-robin routing mode.** Settings drawer now has a routing-mode radio group below the Force-role selector: `Smart routing` (default, orchestrator picks direct/single/parallel per turn) vs `Round-robin` (every turn bypasses the orchestrator's plan-call and fans out parallel to all four specialists — perception, reasoning, action-code, action-structural — then synthesizes through orchestration). The mode persists in `localStorage[lattice.settings.v1]`; each chat-stream request sends `routingMode: 'round-robin'` when active. Server-side, `buildChatOpts` translates the body field into `routing.roundRobin: true` on `ChatSession.send`, which constructs a hardcoded parallel `Plan` and feeds it to the existing executor via a new `prebuiltPlan` argument on `planAndExecute`. Skips the plan-generation call entirely, so round-robin uses one fewer LLM hop per turn than smart routing's parallel branch. See [src/chat/session.ts](src/chat/session.ts), [src/web/server.ts:47](src/web/server.ts), and the `ROUTING_MODES` constant in [src/web/static/app.jsx](src/web/static/app.jsx).
