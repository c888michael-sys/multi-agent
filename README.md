...
**Visual system — "Quantum Lab".**

The UI commits to a single aesthetic: dark cool slate (`oklch(0.125 0.012 245)`), one ice-blue accent (`oklch(0.82 0.14 230)`) doing all the chrome heavy lifting, hairline 6 %-white borders, monospaced instrument-panel labels (`Geist Mono` at 9–11 px with letter-spacing) for status/IDs, and a refined **Bricolage Grotesque** variable display face for the hero headline. The previous warm aurora / rainbow agent palette is gone — the sidebar dots span a narrow blue band, so the screen reads as one piece of pro-tier hardware rather than a kaleidoscope. There is no purple-on-white anywhere.
...
3. **Response** — composer **A** pins to the bottom of the stage. The **newest** response **B** sits at the vertical and horizontal center; older B's stack just above it, dimmed and saturation-pulled, fading via a top-edge mask. At the **top of the canvas** sits a small subtle white pill — the **bar handle**, the catalyst. Hover expands and illuminates it; click or pull-down past ~48 px triggers the catalyst sequence.
4. **Collapsing — the catalyst sequence** (~1.9 s, JS-orchestrated timeline `COLLAPSE_TIMELINE` in `app.jsx`):
   - **0–420 ms — bar slide.** The handle glides down the Y-axis (`mmBarSlide` keyframes), turning from white to accent as it travels, ending in a thin streak at the collision point.
   - **420–920 ms — collision.** Two tokens fly in: **B** (white circle) from the LEFT and **A** (accent square) from the RIGHT, accelerating inward (ease-in cubic), meeting at exact dead center.
   - **920–1010 ms — impact.** A bright radial flash blooms at the collision point; **A locks** at center as the larger anchor (`.mm-anchor-a`); B's geometry dissolves.
   - **1010–1170 ms — shatter.** N particle dots cluster densely around A (N = number of categorized chunks in the newest response, clamped 3–7).
   - **1170–1900 ms — gravity fountain.** Each particle traces a quadratic Bézier from center down/outward to its final settled position in the bottom half of the canvas. The path is drawn as a thin white SVG `stroke-dasharray` trail, "drawn" as the particle travels (offset animated by the same eased progress). Particles **scale up** from 0.18× → 1× as they descend, growing into the full-sized rectangular nodes. Trail opacity fades in the last 20 % of the fountain.
5. **Mindmap (settled, fan)** — A as **active composer** at the geometric center (still typeable — submitting a follow-up from here lands a new B in the stack and unmounts the mindmap). Category nodes are arranged in a **fan in the bottom half only** (angles span 0..π, `useOrbitalPositions` with the `fan` distribution + per-node seeded random jitter + node↔node + node↔composer overlap-resolver pass). SVG dashed accent lines connect every node back to A. Nodes drift gently in place via `mmOrbitNodeDrift`.
...
7. Click (or pull-drag-down past ~48 px) the **bar handle** at the top of the canvas. **Expected:** the bar glides DOWN the Y-axis (~420 ms), turning from white to accent as it travels; tokens **A** (accent square, from right) and **B** (white circle, from left) fly inward and collide at center (~500 ms); a bright radial flash blooms (~90 ms); A locks as the larger anchor square; B shatters into 3–7 particle dots clustered around A (~160 ms); particles arc downward along Bézier paths with thin white SVG trails, scaling up as they descend, and settle in the bottom half as the full-sized category nodes (~730 ms). Total ~1.9 s. Settled state: A composer at center, nodes fanned in the bottom half, dashed accent lines connecting them back to A.
...
**Layout note — Quantum-Lab theme + bar-handle catalyst + fan mindmap (current).** The UI was retuned to a dark cool slate with a single ice-blue accent and Bricolage Grotesque display type — no more warm orange / rainbow agent colors. The expand affordance is now a small white bar handle at the top of the canvas (an iOS-modal pill); clicking or pulling it down triggers a 1.9 s orchestrated catalyst sequence — the bar slides down, A and B tokens fly in horizontally and collide, A locks as the center anchor, B shatters into N particle dots, and the dots arc downward in a Bézier "gravity fountain" with thin white SVG trail lines, scaling up as they descend to settle as the full-sized category nodes. The settled mindmap is a **fan in the bottom half** (not a full 360° orbit), with the composer A still active at the geometric center and SVG dashed lines from each node back to A. The choreography lives in the `CatalystOverlay` component + `COLLAPSE_TIMELINE` in `src/web/static/app.jsx`; the bar handle / token / particle / trail CSS lives in the *Bar handle* / *Catalyst sequence* / *Orbital mindmap* blocks of `src/web/static/style.css`. The response history is the full ordered stack, persisted under `localStorage[lattice.responseStack.v2]`.
