...
- [x] **Tool-capable `action-code` chain for agentic sessions.** Live probe (2026-06) confirmed **Mistral Codestral does NOT support function calling** — it returns empty text even with `tool_choice` forced, which is why early agentic attempts produced blank replies. `buildDefaultRoles({ toolCapable: true })` reorders the `action-code` chain to lead with verified tool-calling models (**Groq Llama 3.3 70B → Cerebras GPT-OSS 120B**), keeping local `ollama:qwen2.5-coder` first in hybrid mode and Codestral/Gemma as last-resort fallback. The CLI `chat` command applies this automatically whenever tools are enabled. Codestral remains the default for non-agentic code generation (multi-agent workflow, web UI) where no tool calls are needed.
