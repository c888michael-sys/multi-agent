...
| **vision** | `gemini:1 → gemini:2` | **Internal, auto-forced** multimodal role. A chat turn that includes pasted/attached images bypasses the text-only multi-agent pipeline and routes here. Gemini Flash only (the wired multimodal provider); no Gemma/text safety net because those can't see images. Not user-selectable and excluded from the orchestrator's planner roster. |
...
Selectable providers: `openrouter`, `nvidia`, `groq`, `cerebras`, `mistral`, `gemini`, `ollama` (whichever have a key set; Ollama needs a running daemon). Model lists are fetched live per provider (OpenRouter keeps its free-tier filtering; the others hit their `/v1/models`, Gemini's ListModels, or Ollama's `/api/tags`). Opening Model routing, changing a provider, or clicking a model dropdown forces a live sync; the UI shows whether the result is live or cached and when it was fetched. The last successful result remains cached for 12h under `~/.multi-agent/models-cache/<provider>.json` only as a labelled fallback when a live refresh fails.

Set it from the web **Settings → Model routing** section (provider dropdown, then model dropdown, per role) or the CLI. The web UI labels these separately as **API provider** and **Model catalogue through provider**, then shows a route receipt: `Active route: OpenRouter API → NVIDIA: Nemotron...` means the NVIDIA-authored model is reached through OpenRouter; `Inactive: API key is missing` means the saved override is not serving and the built-in fallback chain is active. Every click on a model dropdown starts a live refresh, and the server revalidates a newly selected model against another live catalogue response before saving it. A saved model that disappears from a live catalogue is marked inactive. NVIDIA's `/v1/models` response is a catalogue, not a guarantee that every hosted function is enabled for the current account, so NVIDIA endpoint access is verified only when a request runs.
...
- [x] **`chat --allow-bash` — agentic file + shell tools in interactive sessions.** `--allow-bash` and `--workdir` flags now work on `chat` (previously only on `ask`). When tools are enabled, **every turn routes directly through `action-code`'s tool loop**, bypassing the multi-agent/auto/brainstorming pipelines: the model calls `read_file`, `write_file`, `list_dir`, and `bash_exec` inside `--workdir`, sees the results, and produces a final reply — within the persistent session history (up to 10 tool iterations/turn). This direct routing is deliberate — the multi-agent pipeline's final answer always comes from the `action-structural` *formatter*, which describes rather than executes, so agentic tasks like "make a folder" never actually ran a tool. Direct `action-code` routing is the same reliable shape as `ask --allow-bash`. A `[TOOL DIRECTIVE]` is injected so the model executes instead of explaining; each tool call is printed to stderr. Only `action-code` is tool-eligible (giving tool declarations to formatter/planner/checker roles caused Groq 400s). `--workdir` alone gives the three file tools without bash. `/info` shows active tools. Tools are opt-in for `chat` (unlike `ask` which enables file tools by default).
- [x] **`chat` Ctrl+C interrupt (pause).** While a turn is generating, Ctrl+C aborts it and returns to the prompt (the CLI analog of the web stop button) — the partial response is discarded and the pending user message is rolled back so a re-send doesn't double it. At the idle prompt, Ctrl+C exits cleanly. Wired through the existing `AbortSignal` path (`ChatSession.send({ signal })` → router → provider), so an in-flight provider call or backoff sleep is actually cancelled, not just hidden. The agentic tool loop checks the signal between iterations.
- [x] **Richer `action-code` output + higher tool-loop cap.** The `action-code` system prompt now asks for complete, polished, production-quality output (real CSS, layout, responsive design, all necessary files) instead of bare skeletons, and the in-chat `[TOOL DIRECTIVE]` tells the model to finish the whole job across as many tool calls as needed. The tool-loop cap was raised from 10 to 25 iterations/turn so multi-file builds (e.g. a website with separate HTML/CSS/JS) aren't truncated. Note: output quality still depends on the underlying model — Mistral Codestral (cloud) or a larger local model produces richer sites than `qwen2.5-coder:14b`.
- [x] **Intent-narration nudge in the tool loop.** Some models (Codestral especially) reply with narration — "I will create the HTML file..." — as plain text instead of calling a tool, which used to end the turn with nothing built. `looksLikeUnfinishedIntent` detects that pattern and, when no tool has run yet, nudges the model (up to twice) to actually call the tool before accepting a text reply. Tool-activity log lines now print with a leading newline so the REPL spinner's `\r` redraw can't overwrite them.
- [x] **Forced `tool_choice` until first tool runs.** The real fix for "Codestral describes but never acts": the agentic tool loop now sets `tool_choice: "required"` (Mistral `"any"`) on every iteration until at least one tool has actually executed, then switches to `"auto"` so the model can produce its final text reply. Threaded via a new `CompleteOptions.toolChoice` honored by all OpenAI-compatible providers (Mistral/Groq/Cerebras/OpenRouter); `toolChoiceValue` maps `"required"`→`"any"` for Mistral. Providers that don't support it (Gemini/Gemma, Ollama) ignore it and fall back to the narration nudge.
- [x] **Mistral tool-call parser fix + `action-code` switched to Mistral Large.** The "Codestral can't call tools" diagnosis was wrong — the real bug was in `parseOpenAIToolResponse`, which required `type: "function"` on each tool call. OpenAI sends that field; **Mistral omits it entirely**, so every Mistral tool call was silently dropped and surfaced as an empty-text reply. With the parser fixed (accept any call that names a function), live probes confirm **all** Mistral chat models call tools fine (large/small/codestral ✅). `action-code`'s primary is now **`mistral:large`** (`mistral-large-latest` — strongest Mistral model, free on the Experiment plan); override via `MISTRAL_MODEL` in `.env`. `buildDefaultRoles({ toolCapable: true })` no longer reorders the chain — it inserts **Groq Llama 3.3 70B → Cerebras GPT-OSS 120B** as tool-capable fallbacks just before the Gemma safety net, keeping the primary first. The CLI `chat` command applies this automatically whenever tools are enabled.
- [x] **Resilient tool loop.** If a provider errors mid-loop *after* at least one tool has run (e.g. Groq's Llama intermittently emits a malformed `<function=…>` call that the API rejects with a 400), the turn no longer crashes — it returns a summary of the actions completed so far. Verified end-to-end: an agentic turn creates a real file on disk and recovers cleanly from the Groq 400. User interrupts (Ctrl+C) still propagate.
- [x] **`chat` hybrid local toggle.** Both cloud and local (Ollama-prepended) resolvers are built at session startup, so `/local [on|off]` switches between them mid-session without a restart. `--local` starts the session in hybrid mode. `/info` shows the current local state. `OllamaProvider` now implements `completeWithTools` (Ollama `/api/chat` native function calling, supported since v0.3) so local models like `qwen2.5-coder` can drive the agentic tool loop — previously a missing method threw and the pool misread it as a cooldown ("local model cooling"). Some local models (qwen2.5-coder included) emit tool calls as a JSON object in the **content** field instead of the structured `tool_calls` field; `parseInlineToolCalls` detects and coerces those inline shapes (`{name, arguments}`, `{name, parameters}`, `{tool, args}`, OpenAI-style `{function:{...}}`, and arrays) so the loop still executes the tool. It only treats content as a call when it names an offered tool, and skips braces inside string literals.
...
- [x] Stage 6c: Image input — paste, drag-drop, or pick a screenshot in the chat composer. The image is downscaled client-side (canvas, longest edge ≤ 1568px, re-encoded JPEG), sent on the turn as `images: [{mimeType, dataBase64}]`, and routed to the internal multimodal **`vision`** role (Gemini Flash), bypassing the text-only multi-agent pipeline. `ConversationPart.user_text` carries optional `images`; Gemini renders them as `inlineData`. Image turns are stored in history and replayed on later turns — since the OpenAI-compat providers in this app are text-only models (the vision path is Gemini), `historyToOpenAIMessages` flattens an image turn to a text marker (`[user attached N images …]`) rather than an `image_url` payload, which would 400 a non-vision model. Both `/api/chat` and `/api/chat-stream` validate count (≤4), per-image size (~5 MB), and mime (`png/jpeg/webp/gif`). Composer shows thumbnails; the transcript keeps a per-turn count (base64 is never persisted to localStorage). Image paste is web-only.
...
- [x] **Web conversation manager.** Added a first-class `threads` drawer in the web UI with search, open/switch, rename, pin/unpin, duplicate/branch, export JSON, delete, delete-all, refresh, and clear-current actions. Backend session files now expose lightweight metadata (`title`, `pinned`, timestamps, preview, token estimate) through `/api/sessions`, plus PATCH/DELETE/duplicate/export endpoints with session-id validation.
- [x] **Web project file tools — Phases A + B + C (read-only browser, attach-to-chat, diff preview, confirmed writes, model-proposed edit detection).** `files` button in the top nav opens a two-pane drawer: directory listing (left) and file preview (right) with breadcrumb navigation. Blocked paths (`.git`, `node_modules`, `.env`, `*.pem`, `*.key`, symlinks, etc.) are never listed or readable. **Phase A:** `Attach to chat` appends the selected file to the composer's attachment list using the same fenced-block format as the OS file picker. **Phase B:** `Edit` opens an in-drawer textarea; `Preview diff` calls `POST /api/files/diff` and renders a unified diff (green/red line rows); `Apply` calls `POST /api/files/write` with `expectedSha256` to guard against stale-tab overwrites — mismatched hash returns 409. **Phase C:** `detectFileEdits` scans each assistant response for fenced code blocks whose first line is a path comment (`// src/foo.ts`, `# src/foo.py`, etc.) and renders an `apply → path` pill in the response footer; clicking it opens the file drawer, auto-reads the current file, pre-fills the editor with the proposed content, and lands directly on the diff view — human confirmation required before any write. Model never silently writes files. **Phase 0 hardening:** missing-file diffs are explicit `/dev/null` create previews, project folders are only created with an explicit checkbox under the allow-list, paths are case-safe for Windows, request bodies are capped at 3 MiB, and project/file writes require a no-store CSRF token on the loopback-bound server. API: `GET /api/security/context`, `GET /api/files/root`, `GET /api/files?path=`, `GET /api/files/read?path=`, `POST /api/files/diff`, `POST /api/files/write`.

- [x] **Reviewed generated websites — sandboxed preview and undo.** A chat turn with structured multi-file output can now be inspected as a static HTML preview before any disk write. The generated document runs only in an opaque-origin `allow-scripts` iframe with a CSP that blocks network access, forms, base URLs, parent access and app storage. After an approved batch write, the turn keeps a ten-minute, hash-guarded Undo action; it only restores files that have not changed since the write.

- [x] **Builder mode — constrained iterative website creation.** Enable Builder mode in Settings for complex multi-file tasks. The model is routed to the code role with only bounded `list_project`, `read_project_file` and memory-only `stage_file` tools. It cannot receive the real write or shell tools. Tool activity streams with only tool names and relative paths; staged files return to the normal sandboxed preview, review and explicit Apply flow.
- [ ] **UX/UI polish pass (v0.5).** Fifteen improvements across three phases: turn "flight recorder" with per-agent attribution, living template chips, Ctrl+K command bar, paper light theme, reduced-motion support, focus rings, live tab title, jump-to-latest pill, clay-matched syntax theme, empty states, micro-typography, and turn-level retry/edit/branch actions. **Detailed worker plan below: "UX/UI polish pass (v0.5)."**
- [ ] **Edit/regenerate/continue/branch messages.** Add common chat-app controls: edit the last user message and rerun, regenerate assistant response, continue a stopped/truncated response, and branch from a specific turn without overwriting the original. *(Partially delivered by Task C3 of the UX/UI polish plan: retry, edit-and-resend, branch-from-turn.)*
- [~] **Richer attachments.** Image/screenshot input shipped (paste/drag-drop/pick → downscaled → multimodal `vision` role on Gemini; see Stage 6c). Still TODO: PDF extraction, larger-file chunking, folder support, OCR for non-vision fallback, and clearer context-budget estimates before submit.
- [~] **Artifacts and previews.** The Phase 1 backend and Phase 2 review UI now extract structured/legacy file blocks after a completed chat response, group them into one accessible review card, and provide project-bound expiring proposals with lazy diffs, CSRF-protected multi-file apply, conflict checks and file-browser refresh. Sandboxed preview and undo remain next. Specification: [`docs/specs/2026-07-14-web-artifact-workflow.md`](docs/specs/2026-07-14-web-artifact-workflow.md).
...
- [x] **Reasoning model picker.** The reasoning primary is now selectable from OpenRouter's scanned free text-model list. Other roles remain fixed for routing stability; see the future customisation plan below.
- [ ] **Model/provider picker and routing explainability for other roles.** After the reasoning picker proves stable, extend the same catalogue/override pattern to role primaries such as `action-code`, `action-structural`, and `action-repetitive`, plus a per-turn "why this route/model was used" view.
- [ ] **Audio role.** Add a new `audio` role for speech/audio understanding and generation workflows, with the model/provider choice left configurable later. Planned execution: add `audio` to the `RoleName` union and role registry; introduce an `AudioProvider` capability interface instead of forcing audio through text-only `complete()`; add server endpoints and file-attachment handling for audio inputs with size/type validation; expose the role only when an audio-capable provider is registered; add CLI/web diagnostics that show whether audio is unavailable, primary-ready, or falling back. Safety constraints: reject arbitrary remote URLs, cap uploaded file size/duration, store no permanent audio by default, and keep transcripts/artifacts scoped to the active session/project.
- [ ] **Image role.** Add a new `image` role for image understanding and image-generation/editing workflows, with model/provider choice selected later. Planned execution: add `image` to the `RoleName` union and role registry; define image-capable provider interfaces for vision input and generation/edit output; extend web attachments so images are passed as typed parts rather than flattened text; add generated-image artifact previews in the web UI; keep text-only fallbacks explicit instead of pretending unsupported models can inspect images. Safety constraints: validate MIME/size/dimensions, block hidden path traversal through uploaded filenames, avoid persisting generated images unless the user saves them, and show provider/model attribution on every image artifact.
...
- [x] **Switchable project (root folder + pinned file).** Let the user change the active project at runtime in both CLI and web. A project is a root folder plus an optional pinned focus file; one **global** active project at a time, backed by a registry the user manages (create / delete / switch), constrained to an allow-list of base dirs (`MULTI_AGENT_PROJECT_ROOTS`, defaults to the launch dir's parent). Shared store at `~/.multi-agent/projects.json` consumed by both surfaces. CLI: new `project` command (`list`/`current`/`add`/`use`/`remove`/`pin`/`unpin`) plus `/project` slash commands in the chat REPL; `ask`/`task`/`chat`/`serve` default their root to the active project. Web: a project selector at the top of the existing `files` drawer, with the web `WebFileService` rebuilt on switch. New endpoints `GET /api/projects`, `POST /api/projects`, `POST /api/projects/active`, `POST /api/projects/delete`, `POST /api/projects/pin`; `/api/files/root` carries active-project context. **Detailed worker plan below: "Switchable project (root folder + pinned file)."**

- [ ] **UX trust & polish pass (v0.6).** Four independently shippable phases from the 2026-07-16 UX review: accessibility correctness (`inert` on the closed settings drawer, arrow-key tablist, screen-reader turn announcements), first-class error turns (error card + retry + quota countdown with one-shot auto-retry, replacing the fake `(error: ...)` reply text), artifact/Builder workflow polish (auto-prepared review, grouped file list with select all/none, richer turn card with undo countdown + reload persistence, open-files-after-apply, live Builder checklist), and conversation ergonomics (auto-titled threads via the free Cerebras slot, composer draft persistence, markdown thread export, first-run setup card). **Detailed worker plan below: "UX trust & polish pass (v0.6)."**

### Worker implementation plan: UX/UI polish pass (v0.5)

> **For agentic workers:** execute task-by-task, in phase order (A → B → C). Tasks within a phase are independent — any order. Per CLAUDE.md: each task = typecheck + tests green + README checkbox flipped `[x]` + commit + push before starting the next. The frontend is **no-build** (babel-standalone at runtime) — do NOT add npm dependencies, bundlers, or JSX build steps; CDN `<script>` tags in `index.html` are the only way to add libraries. An esbuild parse check in the test suite catches JSX syntax errors. Manually verify each frontend task at `npm run web` → http://localhost:7421.
>
> Key anchors (line numbers approximate — re-grep if drifted): `app.jsx` — `MM_AGENTS` :25 (role→name/color map), `applyAgentEvent` :59, `ConstellationOverlay` :106, `SettingsDrawer` :524, `settingsActiveCount` :857, `Composer` :1182, `ChatTurn` :1346, `statusLabel` :1404, `CodeBlock` :1456, `CatalystOverlay` :1920, `IdleView` :2525, `ResponseStackView` :2619, `DEFAULT_SETTINGS` / `loadSettings` :3056/:3095, `savePersistedStack` :3179, `FileDrawer` :3250, `ConversationDrawer` :3701, `GoalView` :3780, `HeroMindmap` :3938 (owns submit/SSE loop, drawer open-state setters, `stopStream` :4063). `templates.jsx` — `TEMPLATE_DEFS` :10. `style.css` — `:root` vars :7, `.mm-aurora` :62, `.mm-scan` :91, `.mm-template-hint` :462.

#### Phase A — zero-risk polish (CSS + small JS, no backend)

- [x] **A1. `prefers-reduced-motion` support.** Currently zero (`grep -c prefers-reduced-motion style.css` → 0). (1) `style.css`: add `@media (prefers-reduced-motion: reduce)` block pausing `mmdrift` (aurora) and any decorative pulse keyframes — keep the functional streaming caret. (2) `ConstellationOverlay`: check `window.matchMedia('(prefers-reduced-motion: reduce)').matches` at mount; if reduced, draw ONE static frame and skip the RAF loop entirely (keep the resize redraw). (3) Collapse sequence: where `COLLAPSE_TIMELINE` drives the seam-rip (`CatalystOverlay` + its trigger in `HeroMindmap`), if reduced, skip the overlay and fire the completion callback immediately so the mindmap appears without animation. Verify via DevTools → Rendering → emulate reduced motion.
- [x] **A2. Focus-visible rings.** Add `.mm-root :focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; }` plus per-component radius fixes where outlines look wrong. Audit that every interactive element is keyboard-reachable: nav pills, sessions/files/goals buttons, settings gear, drawer rows (threads list, file list), composer buttons, template chips (after A5), turn-footer buttons. Convert clickable `<span>`/`<div>`s to `<button>` or add `role="button" tabIndex={0}` + Enter/Space handlers.
- [x] **A3. Micro-typography.** (1) Cap prose measure: `.mm-turn-ai-bubble { max-width: 72ch; }` (verify against current rule; keep mobile full-width). (2) Add utility `.tnum { font-variant-numeric: tabular-nums; }` and apply to: `.mm-turn-timing`, sidebar RPM/RPD gauge numbers, context counter, `formatCountdown` outputs (goal quota-wait), so ticking numbers don't jitter. (3) AI bubble `line-height` to ~1.6 if lower.
- [x] **A4. Live tab title + favicon.** Base title `Lattice — multi-agent`. In `HeroMindmap`'s SSE loop: on `plan-start` set `document.title = '● planning — Lattice'`; on each `role-start` set `'● ' + stripSlot(role) + ' — Lattice'`; restore base title on done/error/stop (including `stopStream`). `index.html` has no favicon — add `<link id="favicon" rel="icon">` with an inline-SVG data URI (amber dot on transparent, matching `--accent`); during streaming swap to a filled-dot variant, restore after. ~30 lines total.
- [x] **A5. Living template chips.** The 4 chips under the hero (`.mm-template-hint`, rendered in `IdleView` from `TEMPLATE_KEYS`) are dead `<span>`s. (1) Add a `starter` field to each entry in `TEMPLATE_DEFS` (`templates.jsx:10`) — one good example prompt per template (research/code/compare/plan). (2) Make chips buttons: click → `setDraft(starter)` + focus the composer textarea (grep `Composer` for the textarea; expose focus via a ref or `document.querySelector`). (3) CSS: `cursor: pointer`, hover lift (border-color → `--accent-soft`, translateY(-1px)), focus ring per A2. (4) Optional: rotate the composer `placeholder` through the starters every ~5 s while the draft is empty.
- [x] **A6. Empty states.** Three drawers open onto plain nothing. Add a shared `.mm-empty` pattern: a ~44 px inline SVG glyph (1.5 px stroke in `var(--mm-fg-faint)`, single `var(--accent)` dot accent) + one hint line in `--mm-fg-dim`. Copy — threads (`ConversationDrawer`, empty sessions list): "No threads yet — every conversation lands here." Files (`FileDrawer`, empty dir): "Empty folder." Goals (the goals panel empty list): "No goals running. Start one with /goal <what you want done>."

#### Phase B — frontend features (no backend changes)

- [x] **B1. Jump-to-latest pill.** In `ResponseStackView`, track whether the scroll container is near-bottom (`scrollHeight - scrollTop - clientHeight < 80`). While a turn is streaming AND the user has scrolled up, show a floating pill above the composer (`↓ streaming…`, amber border, `position: absolute` within the phase container); click → smooth-scroll to bottom. Hide when at bottom or when streaming ends.
- [x] **B2. Clay-matched syntax theme + code-block chrome.** highlight.js is ALREADY wired (`index.html:33-34` + `CodeBlock` app.jsx:1456) but uses the stock `atom-one-dark` stylesheet, which clashes with the warm palette. (1) Remove the atom-one-dark `<link>` from `index.html`. (2) Add a ~40-line `.hljs-*` block to `style.css`, variable-driven so it follows theme swaps (B4): background transparent; `.hljs-keyword` → `var(--accent)`; `.hljs-string` → `oklch(0.74 0.09 170)`; `.hljs-comment` → `var(--mm-fg-faint)` italic; `.hljs-number/.hljs-literal` → `oklch(0.76 0.08 300)`; `.hljs-title/.hljs-function` → `var(--mm-fg)`; `.hljs-attr/.hljs-property` → `oklch(0.78 0.06 90)`. (3) `CodeBlock` chrome: header row with language chip (mono, faint) + wrap toggle (`white-space: pre` ↔ `pre-wrap` state) + a copy button (reuse the existing `CopyButton` component); style `<pre>` border/radius consistent with bubbles.
- [x] **B3. Command bar (Ctrl/Cmd+K).** New `CommandBar` component mounted in `HeroMindmap`. Global `keydown` listener (cleanup on unmount); Ctrl+K/Cmd+K toggles; Esc closes; ignores the shortcut only when a modal drawer already traps focus. Centered panel (`mm-card-elev` styling, backdrop with slight blur), text input + filtered action list, ↑/↓ + Enter navigation. Actions (each `{ id, label, hint, run }`): new thread; open threads / files / goals / settings (reuse `HeroMindmap`'s drawer open-state setters — grep the GOALS button's `onClick` for the pattern); toggle serious / search / hybrid-local; set routing mode (auto / multi-agent / brainstorming); "start a goal…" (prefills composer with `/goal `); plus the 8 most-recent threads fetched from `GET /api/sessions` on open (selecting one switches session like the threads drawer does). Simple case-insensitive substring filter is fine — no fuzzy lib.
- [x] **B4. Paper light theme.** The Atelier system is one `:root` block (style.css:7-40). (1) Add a `.mm-root[data-theme="paper"]` override block: `--mm-bg: oklch(0.962 0.008 85)`, `--mm-bg-deeper: oklch(0.925 0.010 85)`, `--mm-card-bg: oklch(0.99 0.004 90 / 0.72)`, `--mm-card-elev: oklch(1 0 0 / 0.85)`, `--mm-fg: oklch(0.235 0.012 60)`, `--mm-fg-dim: oklch(0.42 0.014 60)`, `--mm-fg-faint: oklch(0.58 0.012 60)`, `--mm-line: oklch(0 0 0 / 0.08)`, `--mm-line-strong: oklch(0 0 0 / 0.16)`, `--accent: oklch(0.62 0.13 60)`, `--accent-soft/-faint` same hue at 0.4/0.12 alpha. Tune by eye for contrast (AA on body text). (2) Hardcoded non-var colors need paper overrides: `.mm-aurora::before/::after` (drop opacity to ~0.35, warm tint), `.mm-scan` grid lines (white-alpha → black-alpha), nav gradient. (3) `ConstellationOverlay` hardcodes `rgba(125,180,255,…)` — introduce a `--mm-constellation` var read via `getComputedStyle` at mount; define per theme; pass `settings.theme` as a `key` prop so the canvas remounts on switch. (4) Settings: add `theme: 'clay' | 'paper'` to `DEFAULT_SETTINGS` + `loadSettings` validation; segmented control in `SettingsDrawer` (appearance section); count `theme !== 'clay'` in `settingsActiveCount`; apply `data-theme` on the `.mm-root` element in `HeroMindmap`'s render. (5) Check code-theme (B2) and MathJax legibility in paper mode.

#### Phase C — full-stack (attribution plumbing, flight recorder, turn actions)

- [ ] **C1. Provider attribution plumbing.** Goal: every `role-end` event carries which provider actually served the call. (1) `src/roles/resolver.ts`: add an optional trailing `attribution?: CallAttribution` out-param to the public run methods (`runRoleChat`, `runRoleChatStream`, `runRoleWithTools`, both `WithCandidateHistory` variants, `runRole`); inside `runWithStrategy`, on success copy `attribution.providerId ?? candidate.providerId` (and the cross-role path's equivalent) into the caller's object. Backward compatible — existing callers unchanged. (2) `src/chat/session.ts`: extend `ChatProgressEvent`'s `role-end` with `providerId?: string`. In every site that emits `role-end` (direct/single/parallel/synthesis paths in `planAndExecute`, the forced-role path in `send`, `runWithToolLoop`), create `const attr: CallAttribution = {}`, thread it through the `runRoleChat*MaybeFallbackSearch` helpers (add a pass-through param) and `runRoleWithTools`, and emit `providerId: attr.providerId`. (3) `src/agents/multi-agent-workflow.ts`: `WorkflowRuntime.runRole`/`streamRole` gain an optional `attribution?: { providerId?: string }` parameter; `runMultiAgentWorkflow` creates one per call and includes `providerId` in its `role-end` fires; the session's runtime closures pass it through to `runWithToolLoop`. Existing fake runtimes in tests ignore the param — no breakage. (4) The web SSE route forwards progress events verbatim, so `providerId` flows to the client with no server change — verify in `server.ts`'s chat-stream handler. (5) Tests: `roles.test.ts` — attribution filled on primary success AND on within-role fallback (expect the backup's id); `chat-session.test.ts` — collected `role-end` events carry the fake provider's id; run the full suite.
- [ ] **C2. Turn flight recorder + colored pills + quota receipt.** All frontend, consuming C1. (1) Trace accumulation: in `HeroMindmap`'s SSE loop (where `applyAgentEvent` is called), build `entry.trace`: on `role-start` push `{ role, phase, framing, startedAt: Date.now() }`; on `role-end` close the most recent open row for that role (`endedAt`, `ok`, `error`, `providerId`); on `plan` store `entry.planKind`/`entry.planDetail`. Persist `trace` in the localStorage stack (add the field to `savePersistedStack`'s serialized shape; `loadPersistedStack` defaults it to `[]` — additive, no version bump needed). (2) Colored header pills: in `ChatTurn`, when the turn is finished and `entry.trace?.length`, replace the single `servedBy` pill with one pill per traced role — role color from `MM_AGENTS` (add a `colorForRole(role)` helper falling back to `var(--accent)`), label `{shortRole} {seconds}s`. While streaming, tint the existing live pill with the active role's color (`--c` custom prop) and pulse the dot. (3) Recorder rail: a `flight recorder` chevron-toggle in the turn footer expands a left-bordered rail (per-turn `useState`, collapsed by default) — one row per trace entry: color dot, mono role name, phase/framing summary (first ~60 chars of `framing`, else a verb from the phase: planned/researched/acted/checked/repaired/formatted/synthesized), right-aligned `providerId · N.Ns`. Failed rows show the error in `--c: red`-ish. Reserve a nested chip row for future tool calls (`row.toolCalls` — populated when web tools land; see the file-tools plan below). (4) Quota receipt: aggregate `trace` provider counts into a faint mono line in the recorder header — e.g. `mistral:large ×1 · groq:llama-70b ×2`. (5) CSS: `.mm-turn-pill[style*=--c]` colored variant, `.mm-recorder`, `.mm-recorder-row`, matching the mockup aesthetic (11–12 px mono labels, 2 px left border in `--mm-line-strong`). Old persisted entries without `trace` must render exactly as today (servedBy fallback).
- [ ] **C3. Turn actions: retry · edit · branch.** (1) Backend: add `branchSession(id, keepTurns, newId?, storageDir?)` next to `duplicateSession` in `src/chat/session.ts` — load the session JSON, keep the first `keepTurns × 2` history parts, save under the new id (default `${id}-b${Date.now().toString(36)}`, mirroring duplicate's convention), return the summary. Route `POST /api/sessions/:id/branch` `{ turns, newId? }` in `server.ts` with the same session-id validation as duplicate; 400 invalid id / 404 missing / 200 `{ id }`. Tests in `web-server.test.ts`: happy path truncates history to 2×turns, invalid id 400, unknown 404. (2) Frontend: hover-reveal action cluster on each finished turn (next to the copy button): `↻ retry` re-submits `entry.prompt` as a new turn; `✎ edit` sets the composer draft to `entry.prompt` + focuses it; `⑂ branch` POSTs branch with `turns = turnIndex + 1`, then switches to the new session (reuse the threads drawer's open-session flow + `responsesFromSessionHistory`). All disabled while streaming. (3) Flip the separate "Edit/regenerate/continue/branch messages" roadmap bullet to partially-done, noting continue/regenerate-in-place remain.

#### Phase D — deferred integration

- [ ] **D1. Web tool activity in the recorder.** When web file/bash tools land (see "Web project file tools with permissions" plan below and the "Plugin/tool permission UI" roadmap bullet), stream tool-call events through the SSE channel (`{ kind: 'tool-call', role, name, args, ok, summary }`) and render them as the nested chips reserved in C2. Do NOT build this before the permission surface exists.
...
### Worker implementation plan: UX trust & polish pass (v0.6)

> **For agentic workers:** execute task-by-task, in phase order (A → B → C → D); phases are independently shippable, and tasks within a phase are independent unless noted. Per CLAUDE.md: each task = typecheck + tests green + README checkbox flipped `[x]` + `graphify update .` + commit + push before starting the next. The frontend is **no-build** (babel-standalone at runtime) — do NOT add npm dependencies or build steps. Backend tasks use `superpowers:test-driven-development`; frontend tasks are verified with `npx esbuild ./src/web/static/app.jsx --bundle --outfile=NUL` (and the same for `artifact-review.jsx` when touched) plus the manual steps listed per task at `npm run web` → http://localhost:7421.
>
> This plan deliberately does NOT touch the v0.5 Phase C tasks (provider attribution / flight recorder / turn actions on successful turns) — see "UX/UI polish pass (v0.5)" above; either can ship first. B2's retry lives on **error turns only**; when v0.5 C3 later adds its retry/edit/branch cluster to successful turns, both should share one `.mm-turn-actions` styling block.
>
> Key anchors (line numbers approximate — re-grep if drifted): `app.jsx` — `formatCountdown` :314, `SystemStatus` :423, `QuotaBanner` :479, `SettingsDrawer` scrim + aside with `aria-hidden={!open}` :1006–1020 (ref `drawerRef` :1014), `Composer` :1591, `ChatTurn` :1834 (builder activity display :1868, turn foot :1875), builder `tool` event accumulation `toolActivity.slice(-7)` :5067, SSE `error` event capture :5073, turn-finalize fake `(error: ...)` reply :5105, `savePersistedStack` slim projection :3757 (drops `artifact` / `artifactReceipt` / `toolActivity` / errors today), `FileDrawer` :3858 (conditional-rendered, `if (!open) return null` :4116; same for `ConversationDrawer` :4352 and `CommandBar` :4701), threads-drawer export button :4391, `exportSessionUi` :5430, `setFileDrawerPreload` :5551, stop button :5661. `artifact-review.jsx` — `ArtifactTurnCard` :20, `ArtifactReviewDialog` :47, `prepare()` :127, idle "Prepare review" interstitial :221–233, flat file list :239–248, tablist :250–256, `onApplied` receipt with `undoExpiresAt` :190–196. `server.ts` — SSE catch `writeEvent({ kind: "error", ... })` :1333, `/api/usage.json` role `candidates[].cooldownMsRemaining` :1698–1708. `errors.ts` — `AllProvidersExhaustedError` / `NoProvidersConfiguredError` (thrown at call time, not boot — `router.ts:84`, so `serve` runs with zero keys). React is **18.3.1** (`index.html:23`) — the `inert` DOM property must be set via a ref/effect, NOT as a JSX boolean prop (that needs React 19).

**Goal:** close the trust gaps a daily user actually hits — failed turns that masquerade as replies with no recovery path, a review dialog with a needless interstitial and a flat unselectable file list, artifact/undo state that silently vanishes on reload, Builder activity as cryptic text spans, unlabeled threads, drafts lost to reload — plus three accessibility violations observed live in the running app.

#### Phase A — accessibility correctness (frontend only, zero visual change)

- [ ] **A1. `inert` on the closed settings drawer.** Observed live: the closed drawer keeps `aria-hidden="true"` while its buttons stay keyboard-tabbable (WCAG 4.1.2 — focusable content inside a hidden region). Only `SettingsDrawer` is affected: its scrim + aside stay mounted; `FileDrawer` / `ConversationDrawer` / `CommandBar` conditional-render and are fine. Fix: in `SettingsDrawer`, set the DOM property in an effect on the existing `drawerRef` plus a new scrim ref — `el.inert = !open` — keeping `aria-hidden={!open}` as is. Verify: from the idle page, Tab repeatedly — focus must never land on invisible settings controls; open settings and confirm every control is still reachable and Esc/scrim close still works.
...
- `npm run typecheck`, `npm test`, and the esbuild parse checks for `app.jsx` + `artifact-review.jsx` pass after every task; the matching README checkbox flips `[x]` in the same commit that ships each task.
...
multi-agent/
  ...
  src/
    ...
    project/
      store.ts          — project registry (~/.multi-agent/projects.json): read/write/add/remove/switch/pin
    roles/
...
|---|---|---|
| `reasoning` | `qwen3.5:9b` | `openrouter:reasoning` (selected OpenRouter free text model; Qwen3-Next by default), then `gemini:1` / `gemini:2` with `thinking=high` |
| `action-code` | `qwen2.5-coder:14b` | `mistral:large` (`mistral-large-latest`; override via `MISTRAL_MODEL`) |
...
1. **Falls back through the role's own candidate list first** (e.g., reasoning: `openrouter:reasoning` using the selected model → `gemini:1` thinking=high → `gemini:2` thinking=high → `gemma:1/2`).
...
| `/api/sessions` | GET / DELETE | — | GET returns `{ sessions: SessionSummary[] }`, sorted pinned-first then recently updated. DELETE removes all saved session files and returns `{ deleted, sessions: [] }`; the web UI then starts a fresh empty thread. |
