...
- [ ] Web UI: mindmap transition **v3 — robot arm rips the door** (cinematic) **+ persistent canvas dimension** (workspace). The catalyst plays as described — robot arm hooks the seam, yanks the chat off, glowing void blooms, agents fly out — BUT the canvas-white dimension does NOT cross-fade back to the Atelier palette. The mindmap stays on the white canvas indefinitely and only returns to the warm-charcoal theme when the user clicks `COLLAPSE TO THREAD`. The mindmap thus becomes a long-lived working surface rather than a transient peek.
- [ ] Web UI: **drag-rearrange** category nodes on the canvas (snap-to-cluster), AND a **drag-anywhere placement** mode so the user can pin nodes at arbitrary coordinates rather than along the polar ring. Persists per-entry under `localStorage[lattice.responseStack.v2]` so the layout survives reloads. Worth considering alongside the v3 build since the white-canvas workspace is the natural home for free-form placement.
