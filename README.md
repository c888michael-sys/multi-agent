...
| **action-repetitive** | `cerebras:llama3-8b → gemma:1 → gemma:2` | Cerebras Llama 3.1 8B, 1 M tok / day, wafer-scale inference. |
| **mindmap-categorize** | cloud: `gemini:1 → gemma:3 → cerebras:llama3-8b → gemma:2 → gemma:1` · hybrid: `ollama:qwen2.5-coder → gemma:3 → cerebras → gemma:2 → gemma:1` | **A strong JSON-follower leads the chain** (Gemini Flash in cloud mode, local Qwen 2.5 Coder in hybrid mode) because Gemma's free-tier slot is too flaky to be primary — it intermittently returns `404 ... not supported for generateContent` under load (verified: the same slug returns 200 moments later, and ListModels reports it as generateContent-capable, so it's load-shedding, not a config error). `gemma:3` stays reserved for this role as a mid-chain fallback. Powers the web UI's mindmap burst — takes a finished chat reply and emits the structured JSON the orbital renderer needs. Reservation mirrors `gemini:3`'s perception isolation: no other role's chain references `gemma:3`, so even round-robin storms (which hit all four specialists every turn) can never drain the categorize slot. Cerebras sits second as a fast fallback (~2000 tok/sec, JSON-friendly); `gemma:2 / gemma:1` are last-resort. This role is also **mode-agnostic** — it does NOT switch to a local model in hybrid mode, because qwen-coder is already serving `action-code` and being hit twice per round-robin turn was the original "mindmap stuck on structuring…" failure mode. |
...
- [x] **Mindmap categorization rewired — no fictional fallback.** The orbital used to silently render `FALLBACK_DATA` (hardcoded "Option A / Option B" / "Origin and context" / "Prepare → Execute → Learn") whenever both the Cerebras prefetch AND the local markdown-heading splitter failed. That made bursts look randomly seeded with content the model never produced. The new flow:
  1. After the final answer lands, `prefetchMindmapData(entry)` ships the assistant's full reply to a categorizer role with a "preserve every detail" prompt. **Hybrid mode routes to `action-code` (qwen-coder, local); cloud mode routes to `action-repetitive` (Cerebras).** The prefetch's promise is stored in a ref so the burst handler can await it.
  2. On click → if `entry.data` is valid: instant burst. If `dataLoading`: BarHandle shows `structuring…` (pulsing) and `expand()` awaits the pending promise. If `dataError` or the await yields nothing valid: **no fictional fallback** — the seam label flips to `"couldn't structure this reply — try rephrasing or burst a later turn"` for 3.5 s and the user stays on the chat phase.
  3. `FALLBACK_DATA` is still defined in [templates.jsx:78](src/web/static/templates.jsx) but no longer reachable from the live code path — kept as inert reference for future template renderers. The local markdown-heading splitter (`deriveMindmapData`) is also no longer auto-called at burst time; it's a backup categorizer the orchestrator could opt into in the future, not a quietly-firing safety net.
...
- [x] **Round-robin routing mode.** Settings drawer routes both `routingMode` and `forceRole` through a single merged dropdown (`ROUTING_OPTIONS` in [src/web/static/app.jsx](src/web/static/app.jsx)). Top entries are the two meta-modes — `auto (smart routing)` (orchestrator picks direct/single/parallel per turn) and `round-robin (default)` (every turn bypasses the orchestrator's plan-call and fans out parallel to all four specialists — perception, reasoning, action-code, action-structural — then synthesizes through orchestration). The remaining entries pin every turn to a specific role's chain (the prior "Force role" select). **Round-robin is now the default** so a fresh session feels multi-agent without setup. The underlying wire format is unchanged: each chat-stream request sends `routingMode: 'round-robin'` when active and `forceRole: <role>` for pinned roles. Server-side, `buildChatOpts` translates the body fields into `routing.roundRobin: true` / `routing.forceRole` on `ChatSession.send`, which constructs a hardcoded parallel `Plan` and feeds it to the existing executor via a `prebuiltPlan` argument on `planAndExecute`. Skips the plan-generation call entirely, so round-robin uses one fewer LLM hop per turn than smart routing's parallel branch. See [src/chat/session.ts](src/chat/session.ts) and [src/web/server.ts:47](src/web/server.ts).
- [x] **Hybrid-mode local-detection gate.** Toggling "Hybrid local models" ON in the settings drawer now pings a new `GET /api/ollama-health` route, which probes the configured Ollama daemon's `/api/tags` endpoint (1.5 s timeout) and reports `{ reachable, installed, missing }`. If the daemon isn't running OR the two required models (`deepseek-r1:32b`, `qwen2.5-coder:latest`) aren't pulled, the toggle is **refused** with an in-drawer error explaining exactly what's missing, instead of silently falling through to cloud fallbacks on every reasoning / action-code turn (which used to make the mindmap categorize call appear to hang or fail). On app mount, if `useLocal=true` is persisted from a prior device but Ollama isn't reachable on this one, the toggle is auto-cleared and a transient banner (`mm-quota-warn` style) explains why. The categorizer prefetch (qwen-coder in hybrid mode, Cerebras in cloud mode) therefore always routes to a reachable role. See `onToggleLocal` and the mount-time `useEffect` in [src/web/static/app.jsx](src/web/static/app.jsx), plus the `/api/ollama-health` handler in [src/web/server.ts](src/web/server.ts).
- [x] **Mindmap categorization survives prose-wrapped JSON + adds timeouts.** The categorizer prefetch (qwen-coder in hybrid mode, Cerebras in cloud mode) used to parse responses with `String(reply).replace(/```json|```/g, '').trim()` + `JSON.parse`. Local models reliably broke that: qwen-coder typically prefixes its JSON with a sentence of prose, DeepSeek-R1-class models wrap their output in `<think>…</think>` chain-of-thought blocks, and either case caused the parser to bail → `dataError=true` → mindmap refused to activate in hybrid mode. Replaced with `extractFirstJsonObject(text)` which (1) strips `<think>…</think>` blocks (multi-line, case-insensitive), (2) strips `` ```json ``  and bare `` ``` ``  fences, (3) walks the cleaned text tracking brace depth and JSON-string state (so braces inside string literals don't confuse the scan), and (4) returns the first balanced `{…}` block. Also added a 120 s `AbortController` timeout on `prefetchMindmapData`'s fetch and a 180 s default `requestTimeoutMs` on every `OllamaProvider` call — without these, a stalled daemon would leave the prefetch promise pending forever and the BarHandle stuck on `structuring…`. Verified against six representative noisy outputs (bare JSON, fenced JSON, prose+JSON+trailer, `<think>` blocks, braces-inside-string-literals, no-JSON-at-all) — all parse correctly or return null cleanly. See `extractFirstJsonObject` near the top of [src/web/static/app.jsx](src/web/static/app.jsx), `prefetchMindmapData` further down, and `fetchWithTimeout` in [src/providers/ollama.ts](src/providers/ollama.ts).
...
- [x] **Gemma intermittent-404 failover + mode-aware categorize routing.** Two linked fixes for the "could not structure the mindmap" report:
  1. **404 is now failover-able.** `gemma-3-27b-it` on the free tier intermittently returns `404 ... is not found for API version v1beta, or is not supported for generateContent` — load-shedding dressed up as not-found (confirmed via direct probe: same slug alternates 200/404 within seconds, and ListModels reports it generateContent-capable). `GeminiProvider.isRateLimitError` previously returned `false` for 404, so the resolver threw out of the whole role instead of rotating to the next candidate. Now a 404-not-found/not-supported is treated as a transient failover-able condition, so a Gemma load-shed rotates to Cerebras (or the next chain slot) instead of failing the burst. A genuinely wrong slug just exhausts the chain → `AllProvidersExhaustedError`, the correct failure mode.
  2. **mindmap-categorize leads with a reliable JSON model.** Gemma was the primary categorizer and too flaky for it. Now `buildDefaultRoles` puts Gemini Flash (`gemini:1`) at the front in cloud mode and local Qwen 2.5 Coder (`ollama:qwen2.5-coder`) at the front in hybrid mode; `gemma:3` + Cerebras remain as fallback. `rolesFor` now routes BOTH modes through `buildDefaultRoles` (previously cloud mode used raw `DEFAULT_ROLES` and skipped the prepend). Verified live: a cloud categorize call returns valid JSON (`gemini:1`, 3 phases parsed) where `gemma:3` had been 404ing. See `isRateLimitError` in [src/providers/gemini.ts](src/providers/gemini.ts), `buildDefaultRoles` in [src/roles/default-registry.ts](src/roles/default-registry.ts), and `rolesFor` in [src/cli.ts](src/cli.ts).
- [x] **LoadingView state-accumulation fix.** The per-agent status rows used to be derived via `useMemo` from `liveStatus` (the latest SSE event) alone, which meant every new event blanked the prior roles back to `queued`. In round-robin mode (which fires `plan-start → plan → role-start perception → role-end perception → role-start reasoning → ...` in sequence) you'd only ever see the most-recent 1–2 rows lit up; the rest looked frozen. Switched to `useState` + `useEffect` that applies each event incrementally to the prior map, resetting only on `plan-start` (= new turn boundary) or when `liveStatus` goes `null` (= no turn in flight). Now the full progression is visible: Orchestrator → planning → plan: parallel → all four specialists thinking → all four done → Orchestrator synthesizing → done. See `LoadingView` in [src/web/static/app.jsx](src/web/static/app.jsx).
- [x] **Reserved `gemma:3` slot for the mindmap-categorize role.** Mirrors the `gemini:3 → perception` reservation pattern but for the mindmap categorizer. New `mindmap-categorize` role added to the `RoleName` union with chain `gemma:3 → cerebras:llama3-8b → gemma:2 → gemma:1`. **`gemma:3` is removed from every other role's safety net** so chat traffic on any role cannot drain the categorize slot — even round-robin storms (which fire all four specialists per turn). The frontend's `prefetchMindmapData` now ALWAYS routes to this role regardless of hybrid mode; previously, hybrid mode aliased the prefetch to `action-code` (qwen-coder) which got hit twice per round-robin turn — once for the action-code parallel call and again for the categorize — causing the categorize call to either queue forever or truncate against Ollama's default context. Reserved Gemma is mode-agnostic, so the mindmap stays responsive even when both local Ollama models are saturated. See the `mindmap-categorize` entry in [src/roles/default-registry.ts](src/roles/default-registry.ts) and the `prefetchMindmapData` change in [src/web/static/app.jsx](src/web/static/app.jsx).
- [x] **Max-context defaults for local Ollama models.** Each `LOCAL_OLLAMA_MODELS` entry now carries a `numCtx` field at its model's native maximum: `deepseek-r1:32b` → 65 536 tokens (full 128 K halved to a 40 GB-class GPU target), `qwen2.5-coder:latest` → 32 768 tokens (its full native window). Ollama's daemon default is 2 048, which was silently truncating long chat replies + the categorize schema and producing garbage output. The `OLLAMA_NUM_CTX` env var still globally overrides for memory-constrained hardware (e.g. set to `8192` on smaller GPUs). Also added a 180 s `requestTimeoutMs` via a new `fetchWithTimeout` helper in `OllamaProvider` so a stalled daemon never leaves a request pending indefinitely. See [src/providers/ollama.ts](src/providers/ollama.ts) and `loadOllamaProviders` in [src/config.ts](src/config.ts).
