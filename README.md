...
5. **Mindmap (settled, full cycle)** — A as **active composer** at the geometric center (still typeable — submitting a follow-up from here lands a new B in the stack and unmounts the mindmap). Category nodes are distributed around A in a **full 360° cycle** (angles span 0..2π, starting at 12 o'clock; `useOrbitalPositions` with seeded random angle/radius jitter + a node↔node + node↔composer overlap-resolver pass that holds the no-overlap invariant). More categories naturally expand around the prompt box, never crowding. SVG dashed accent lines connect every node back to A. Nodes drift gently in place via `mmOrbitNodeDrift`.
