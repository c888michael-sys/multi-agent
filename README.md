...
| **vision** | `gemini:1 → gemini:2` | **Internal, auto-forced** multimodal role. A chat turn that includes pasted/attached images bypasses the text-only multi-agent pipeline and routes here. In **hybrid mode**, the configured local reasoning model is prepended and receives the image through Ollama's native `images` field; Gemini remains the fallback. Not user-selectable and excluded from the orchestrator's planner roster. |
...
- [x] Stage 6c: Image input — paste, drag-drop, or pick a screenshot in the chat composer. The image is downscaled client-side (canvas, longest edge ≤ 1568px, re-encoded JPEG), sent on the turn as `images: [{mimeType, dataBase64}]`, and routed to the internal multimodal **`vision`** role, bypassing the text-only multi-agent pipeline. `ConversationPart.user_text` carries optional `images`; Gemini renders them as `inlineData`, while hybrid Ollama turns send raw base64 through its native `images` field so a multimodal local reasoner can inspect them. Image turns are stored in history and replayed on later turns — OpenAI-compatible text-only providers flatten an image turn to a text marker (`[user attached N images …]`) rather than an `image_url` payload that they may reject. Both `/api/chat` and `/api/chat-stream` validate count (≤4), per-image size (~5 MB), and mime (`png/jpeg/webp/gif`). Composer shows thumbnails; the transcript keeps a per-turn count (base64 is never persisted to localStorage). Image paste is web-only.
...
- [x] **Safer local Ollama defaults + installed-tag fallback.** Hybrid mode defaults to the smaller local pair (`qwen3.5:9b` for reasoning/vision, `qwen2.5-coder:14b` for coding) with 32K context each. Users can opt up or down with `OLLAMA_REASONING_MODEL`, `OLLAMA_CODER_MODEL`, `OLLAMA_NUM_CTX`, `OLLAMA_REASONING_NUM_CTX`, and `OLLAMA_CODER_NUM_CTX`. Local generation has no fixed wall-clock deadline by default; the Stop button and client disconnect still abort the underlying Ollama fetch, and 15-second SSE heartbeats protect quiet model-load/thinking periods from idle proxy closure. Set a positive `OLLAMA_REQUEST_TIMEOUT_MS` only to opt into an automatic cutoff. If Ollama has a compatible variant tag pulled for the requested size (for example `qwen3.5:9b-q4_K_M` instead of the configured `qwen3.5:9b`), the provider resolves that installed tag after the first 404 and retries instead of failing the multi-agent turn. It will not silently use `qwen2.5-coder:32b` when the configured default is `qwen2.5-coder:14b`. See [src/providers/ollama.ts](src/providers/ollama.ts) and `loadOllamaProviders` in [src/config.ts](src/config.ts).
...
- [x] **Web conversation manager.** Added a first-class `threads` drawer in the web UI with search, open/switch, rename, pin/unpin, duplicate/branch, JSON and Markdown export, delete, delete-all, refresh, and clear-current actions. First completed turns receive a concise generated title without blocking the reply, and composer drafts persist per thread. Backend session files expose lightweight metadata (`title`, `pinned`, timestamps, preview, token estimate) through `/api/sessions`, plus PATCH/DELETE/duplicate/export endpoints with session-id validation.
...
- [x] **Builder mode — constrained iterative website creation.** Enable Builder mode in Settings for complex multi-file tasks. The model is routed to the code role with only bounded `list_project`, `read_project_file` and memory-only `stage_file` tools. It cannot receive the real write or shell tools. Tool activity streams into a persistent staged-file checklist; staged files return to the grouped, auto-prepared sandboxed preview, review and explicit Apply flow.
- [x] **UX trust and recovery pass (v0.6).** Failed streams are first-class typed error turns with text/image-safe retry, quota cooldown countdowns and one-shot auto-retry. Artifact receipts and Builder activity survive reloads, generated-file undo expires visibly, hidden settings are inert, artifact tabs support keyboard navigation, and keyless installs show setup guidance. Specification: [`docs/specs/2026-07-16-ux-trust-and-polish-pass.md`](docs/specs/2026-07-16-ux-trust-and-polish-pass.md).
...
- [x] **Artifacts and previews.** The review-first workflow extracts structured/legacy file blocks, auto-prepares grouped selectable proposals with lazy diffs and a sandboxed preview, applies the selected batch through CSRF-protected project-bound transactions, refreshes the file browser, and supports an explicit expiring undo action that survives reload. Specification: [`docs/specs/2026-07-14-web-artifact-workflow.md`](docs/specs/2026-07-14-web-artifact-workflow.md).
...
This starts two independent loopback listeners. Open `http://127.0.0.1:7421` on the host PC for the full interface, including local projects, files, Builder tools, settings, goals, and saved sessions. Tailscale proxies only port `7422`. Its exact server-side allowlist exposes chat, streamed chat, mindmap completion, quota/provider status, Ollama health, hybrid/search/thinking/routing/theme controls, provider/model selection, and editable role behaviour. It does **not** expose API keys or the role-instruction file path.

On the shared listener, project/file APIs, Builder tools, artifacts, goals, autonomous tasks, security context, and saved-session APIs all return 404, and the browser omits those controls. Shared chat history is kept in a bounded in-memory cache, never loaded from or written to the session directory, and disappears when the server restarts. Theme, hybrid, search, thinking, and routing choices are browser-local to the friend's device. Provider/model overrides and role instructions are intentionally global host application settings: a trusted friend can change them for subsequent calls made by everyone using the server, and can consume the host's provider or local-model capacity.
...
Share the HTTPS URL printed by `tailscale serve status` only with a friend who has been granted access to the device in your tailnet. Do **not** use `tailscale funnel`: Funnel is public internet exposure, and the shared listener does not add application-level authentication or per-user rate limiting. The friend can see quota/cooldown status and manage non-file model settings by design, but cannot reach the host workspace through this listener.
...
If you have an Ollama daemon running on the same machine (or on your LAN), you can route reasoning, image understanding, and coding to local models instead of the cloud:
...
| `vision` | `qwen3.5:9b` | `gemini:1`, then `gemini:2` |
| `action-code` | `qwen2.5-coder:14b` | `mistral:large` (`mistral-large-latest`; override via `MISTRAL_MODEL`) |

All other roles (orchestration, perception, action-structural, action-repetitive) continue to use their cloud chains regardless of this toggle — perception in particular needs Google Search grounding and cannot be swapped to a local model. The configured reasoning model must support vision to inspect images locally; otherwise use cloud mode for image turns.
...
Override the daemon URL with `OLLAMA_HOST=http://other-host:11434` in `.env` if Ollama runs on a different machine. To use a bigger/smaller local pair without code changes, set `OLLAMA_REASONING_MODEL` and/or `OLLAMA_CODER_MODEL`; the reasoning override also serves hybrid image turns. Use `OLLAMA_NUM_CTX` for both roles or `OLLAMA_REASONING_NUM_CTX` / `OLLAMA_CODER_NUM_CTX` when mixing sizes. For example, a larger reasoner plus 14B coder can use `OLLAMA_REASONING_MODEL=qwen3.5:32b`, `OLLAMA_REASONING_NUM_CTX=16384`, `OLLAMA_CODER_MODEL=qwen2.5-coder:14b`, and `OLLAMA_CODER_NUM_CTX=32768`. Local generation has no automatic deadline by default; Stop and disconnect still cancel it. Set `OLLAMA_REQUEST_TIMEOUT_MS` to a positive number only if you want a fixed cutoff.
