...
The prompt's *type* is detected via keyword heuristics (`research|find|sources|...` -> research, `code|implement|function|...` -> code, `compare|vs|tradeoffs|...` -> compare, anything else -> plan). The chat response is not forced into that schema. When the user bursts to mindmap, the frontend locally derives nodes from the already-rendered markdown answer (`deriveMindmapData(...)` in `templates.jsx`). It first uses markdown headings, then bold/numbered label lines such as `**Risks:** ...`, then paragraph chunks. This avoids a second model call, saves quota, and prevents the previous "animation finishes but mindmap is blank/stuck" failure mode. It is still heuristic; a separate high-quota categorizer role would categorize better, but would spend an extra model call unless precomputed after each chat response.
...
| `/api/chat` | POST | `{ sessionId, message }` | `{ reply, servedBy, plan, summarizedTurns, tokenEstimate, budgetPct, turns, ... }` — multi-turn `ChatSession` shape used by the frontend |
| `/api/usage.json` | GET | — | `{ mode, providers: [...], roles: { <role>: {providerId, primaryProviderId, registered, status, fallback, remainingPct, cooling, candidates} } }` — machine-readable counterpart to `/api/usage`, polled by the sidebar |
