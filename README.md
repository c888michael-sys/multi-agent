...
**Visual system — refined cool.**

Dark cool slate (`oklch(0.125 0.012 245)`) with a single low-chroma steel-blue accent (`oklch(0.80 0.06 230)`) doing the chrome heavy lifting. Chroma is deliberately kept low — the design reads as a refined document tool rather than a game UI, with glows and box-shadows dialed down to near-zero. Hairline 6 %-white borders, monospaced instrument-panel labels (`Geist Mono` at 9–11 px with letter-spacing) for status/IDs, and a **Bricolage Grotesque** variable display face for the hero. Sidebar agent dots span a narrow blue band, so the screen reads as one piece of pro-tier hardware rather than a kaleidoscope.

**Response model — full markdown, never concised.**

Every prompt routes through `/api/chat` — the same orchestrator-driven smart-routing pipeline as the CLI's `task` and `chat` commands (orchestrator plans → direct / single / parallel → specialists → synthesis). The reply lands as **natural markdown** (the renderer handles `# / ## / ###` headings, **bold**, *italic*, `inline code`, ```fenced code blocks```, `- / * / 1.` lists, `> blockquotes`, and `[links](url)`). The B card shows everything the orchestrator wrote — **nothing is ever concised** in the response itself. The mindmap is a separate visual mode that **categorizes** the existing response (splits on H2/H3 headings, or chunks by paragraphs if no headings exist); it doesn't ask the model to re-summarize. One persistent `sessionId` per browser tab carries chat history between turns.
...
3. **Response** — composer **A** pins to the bottom of the stage. The **newest** response **B** sits at the vertical and horizontal center, rendering the orchestrator's full markdown reply (scrollable inside the card when long); older B's stack just above it, dimmed, saturation-pulled, and clipped to a teaser height via a top-edge mask. At the **top of the canvas** sits a small subtle white pill — the **bar handle**, the catalyst. Hover expands and illuminates it; click or pull-down past ~48 px triggers the catalyst sequence.
...
5. **Mindmap (settled, full cycle)** — A as **active composer** at the geometric center (still typeable — submitting a follow-up from here lands a new B in the stack and unmounts the mindmap). Category nodes are derived from the response's markdown by `extractCategoriesFromMarkdown`: each `##` heading (or `###` if there are no H2s) becomes a node titled by the heading, body = the markdown between that heading and the next. Responses with no headings get chunked by paragraphs (≤5 nodes). Nodes are distributed around A in a **full 360° cycle** (angles span 0..2π, starting at 12 o'clock; `useOrbitalPositions` with seeded random angle/radius jitter + node↔node + node↔composer overlap-resolver). SVG dashed accent lines connect every node back to A; nodes drift gently in place via `mmOrbitNodeDrift`.
6. **Imploding** (~440 ms) — nodes scale toward A, lines fade, the response stack re-materializes.

The orbital mindmap always visualizes the **newest** response in the stack. Categorization is purely a client-side reshaping of the already-fetched markdown — no second API call, no re-summarization.
...
8. **Type a follow-up prompt into A while still in the orbital view** and press Enter / click send. **Expected:** the orbital unmounts; you see the loading phase; then the response stack returns with a new B added (and the orbital is collapsed by definition since we left the mindmap phase).
9. Click *collapse* in the mindmap thread-strip. **Expected:** nodes implode toward A, lines fade, then the response stack re-materializes with the cards staggering back into place.
10. Try a code prompt, a comparison prompt, a planning prompt. Each should pick a different template and produce different orbital nodes (files for code, targets for compare, phases for plan).
11. Reload the page. **Expected:** the entire response stack is restored from `localStorage[lattice.responseStack.v2]` and you land back on the response phase with the newest at vertical center.

**Layout note — refined-cool theme + labeled-token catalyst + 360° mindmap (current).** Dark slate background, low-chroma steel-blue accent (chroma intentionally dialed down so the UI reads as a pro document tool, not a game). The expand affordance is a small white bar handle at the top of the canvas; clicking or pulling triggers a 1.9 s orchestrated catalyst sequence — the bar slides down, two **labeled text-box tokens** (`output` from the left, `mindmap` from the right) fly horizontally inward and touch at center, the `mindmap` token re-mounts as the centered anchor, and N **mini text-box particles** (each pre-rendered with the eventual category's label + content preview) travel along Bézier arcs to their final orbital positions. The trail is rendered as a **partial Bézier via de Casteljau subdivision** so the trail's endpoint always equals the box's current position — the line never overshoots. The settled mindmap is a **full 360° cycle** around the composer A (not the previous fan): more categories naturally expand into a complete ring. The choreography lives in the `CatalystOverlay` component + `COLLAPSE_TIMELINE` in `src/web/static/app.jsx`; the bar handle / token / particle / trail CSS lives in the *Bar handle* / *Catalyst sequence* / *Orbital mindmap* blocks of `src/web/static/style.css`. The response history is the full ordered stack, persisted under `localStorage[lattice.responseStack.v2]`.
