...
- [x] Stage 6c: Image input — paste, drag-drop, or pick a screenshot in the chat composer. The image is downscaled client-side (canvas, longest edge ≤ 1568px, re-encoded JPEG), sent on the turn as `images: [{mimeType, dataBase64}]`, and routed to the internal multimodal **`vision`** role (Gemini Flash), bypassing the text-only multi-agent pipeline. `ConversationPart.user_text` carries optional `images`; Gemini renders them as `inlineData`, OpenAI-compat providers as `image_url` data URLs (so a vision-capable override model also works). Both `/api/chat` and `/api/chat-stream` validate count (≤4), per-image size (~5 MB), and mime (`png/jpeg/webp/gif`). Composer shows thumbnails; the transcript keeps a per-turn count (base64 is never persisted to localStorage). Image paste is web-only.
