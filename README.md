...
Cross-device note: GitHub does not include `.env`. On every device that runs the CLI or web server, create its own `.env` with the cloud keys (`GEMINI_KEY_1`, etc.). If local Ollama runs on a different device from the web server, set `OLLAMA_HOST=http://<that-device-ip>:11434` in the web server device's `.env`. To diagnose what a device actually loaded, run `npm run cli -- doctor`.
