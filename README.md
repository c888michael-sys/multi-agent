...
- [x] **`chat --allow-bash` — file + shell tools in interactive sessions.** `--allow-bash` and `--workdir` flags now work on `chat` (previously only on `ask`). When enabled, specialist roles run through a tool-calling loop: model can call `read_file`, `write_file`, `list_dir`, and `bash_exec` inside `--workdir`, see the results, and produce a final reply — all within the persistent session history. Each tool call is printed to stderr. `--workdir` alone gives the three file tools without bash. `/info` shows active tools. Tools are opt-in for `chat` (unlike `ask` which enables file tools by default).
