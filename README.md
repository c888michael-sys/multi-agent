...
- [x] **Web project file tools — Phase A (read-only browser + attach-to-chat).** `files` button in the top nav opens a two-pane drawer: directory listing (left) and file preview (right) with breadcrumb navigation. Blocked paths (`.git`, `node_modules`, `.env`, `*.pem`, `*.key`, etc.) are never listed or readable. `Attach to chat` appends the selected file to the composer's attachment list using the same fenced-block format as the OS file picker. API: `GET /api/files/root`, `GET /api/files?path=`, `GET /api/files/read?path=` — path traversal returns 403, oversized files 413, binary files 415. Phase B (diff preview + confirmed writes) and Phase C (model-assisted reads) are next.
